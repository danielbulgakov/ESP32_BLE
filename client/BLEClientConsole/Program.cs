...
public class BluetoothObserver
{
    ...
    private async void Watcher_Received(BluetoothLEAdvertisementWatcher sender, BluetoothLEAdvertisementReceivedEventArgs args)
    {
        if (isFindDevice)
            return;
        if (args.Advertisement.LocalName.Contains("ESP32"))
        {
            isFindDevice = true;
            ...
            if (result.Status == GattCommunicationStatus.Success)
            {
                ...
                foreach (var service in services)
                {
                    ...
                    if (!service.Uuid.ToString().StartsWith("1c697d04-f628-11ec-b939-0242ac120002"))
                    {
                        continue;
                    }
                    ...
                    if (characteristicsResult.Status == GattCommunicationStatus.Success)
                    {
                        ...
                        {
                            if (!characteristic.Uuid.ToString().StartsWith("1c697fd4-f628-11ec-b939-0242ac120002"))
                            {
                                continue;
                            }
...
class MainClass {
    ...
    {
        BluetoothObserver bo;
        bo = new BluetoothObserver();
        bo.Start();
    }
}
